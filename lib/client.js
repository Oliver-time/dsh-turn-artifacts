	...
	factory: (require) => {
		...
		function ensureStyle() {
			...
			style.textContent = [
				...
				// The menu's own transient line. It needs to be visibly not-an-item:
				// it is a status, and clicking it must not look like an option. It also
				// carries a full path when the file is gone, so it has to wrap.
				".dta-status{display:block;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;padding:4px 8px;pointer-events:none;word-break:break-all;max-width:360px}",
			].join("");
