window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Whether the Host's Windows reveal can still find this path.
		 *
		 * `revealNativePath` does not hand Explorer a filesystem path. It builds
		 * `pathToFileURL(path)`, percent-encodes it, and runs
		 * `explorer.exe /select, <that URL>` — and Explorer does not percent-decode
		 * the way a browser does. Measured on this machine, one call per case, asking
		 * the shell which window exists afterwards:
		 *
		 * | file name                       | as shipped |
		 * | ------------------------------- | ---------- |
		 * | `plain.txt`                     | folder opens |
		 * | `with space.txt` (`%20`)        | folder opens |
		 * | `comma,file.txt` (`%2C`)        | folder opens |
		 * | `暑期科研汇报_v13.pptx` (`%E6…`)  | **no window at all** |
		 * | `中文目录\report.pptx`           | **no window at all** |
		 *
		 * So a multi-byte escape is fatal — the non-ASCII characters this project's
		 * own artifacts are full of — while single-byte escapes survive. The caller
		 * uses this to open the containing folder instead, which the Host does
		 * through PowerShell's `Invoke-Item -LiteralPath` and which handles every
		 * name above.
		 *
		 * This is a property of the Host, not of the browser's platform; the check is
		 * deliberately conservative because the client has no way to ask which OS is
		 * on the other end (`remote.$host` publishes loopback state only).
		 *
		 * @param path - the path that will be revealed.
		 * @returns true when Explorer can still resolve the encoded form.
		 */
		function hostRevealCanFind(path) {
		 ...
		 * the path has to be absolute before it leaves the browser, and the caller is
		 * what makes it so.
		 *
		 * A reveal the Host cannot perform degrades to opening the containing folder
		 * rather than to nothing — see {@link hostRevealCanFind} for the measurement
		 * behind that. The folder is opened through the same RPC with the `action`
		 * omitted, so the fallback needs no new native surface.
		 *
		 * @param ctx - client root context carrying the session remote.
		 * @param sessionId - session whose workspace resolves the path.
		 * @param path - the tab's path, absolute.
		...
		async function openPathNatively(ctx, sessionId, path, action) {
			...
			const folder = action === "reveal" && !hostRevealCanFind(path) ? parentFolderOf(path) : undefined;
			const request = folder === undefined
				? (action === "reveal" ? { path, action } : { path })
				: { path: folder };
			try {
				...
				return NATIVE_ACTION_LABELS[folder === undefined ? (action === "reveal" ? "reveal" : "open") : "folder"];
			} catch (error) {
		...
		function FileTabMenuItems(props) {
			...
			const run = (action) => {
				...
				const done = openPathNatively(ctx, parts.sessionId, absolute, action);
				if (typeof done?.then === "function") done.then(finish, () => finish("打开失败"));
		...
		function ensureStyle() {
			...
			style.textContent = [
				...
				// it is a status, and clicking it must not look like an option.
				".dta-status{display:block;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;padding:4px 8px;pointer-events:none}",
			].join("");
		...
		exports.hostRevealCanFind = hostRevealCanFind;
