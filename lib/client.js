window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * — when the deliverables surface learned to open files as `dsh-resource://`
		 * addresses in the right Sidebar, and that sessionId is what those addresses
		 * are keyed by. A wrapper that named only `owner` silently handed the shipped
		 * resolver an `undefined` session, so the official vocabulary answered
		 * nothing and the plugin broke the very feature it was extending. Rest
		 * arguments make the wrapper signature-independent: whatever the shipped
		 * method takes now or later arrives intact.
		 *
		 * @param ctx - client root context.
		 ...
		 * messages. That is real work for a large log. The honest fix is to page
		 * lazily as the reader scrolls up, or to stop depending on the window at all
		 * (the artifact index is turn-scoped, so a whole-log read would be needed);
		 * neither is done. This flag is the switch between "historical mentions work"
		 * and "the app is cheap to open".
		 */
