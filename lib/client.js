window.__ModuleLoader__.load({
	...
	factory: (require) => {
				...
				return {
					open: () => {
						...
						if (sidebar !== undefined) {
							sidebar(sessionFileAddress(sessionId, target));
							return;
						}
		 ...
		 * its presence is exactly the question "does this deployment have one".
		 *
		 * @param ctx - client root context.
		...
		function sidebarOpener(ctx, sessionId) {
			...
			if (sidebar === undefined || typeof sidebar.openResource !== "function") return undefined;
			return (address) => {
				try {
					sidebar.openResource(address);
				} catch {
					// A wiring problem on the sidebar side must not swallow the click.
				}
			};
		}
