window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * The containing directory of a `/`- or `\`-separated path.
		 *
		 * @param path - an absolute path whose folder is wanted.
		 * @returns the folder, or undefined when the path names no location above it.
		 */
		function parentFolderOf(path) {
			...
			// A drive root keeps its separator: `C:` on its own means "the current
			// directory on C", which is not the folder a file at `C:\a` lives in.
			if (/^[A-Za-z]:$/.test(parent)) return `${parent}/`;
			return parent === "" ? "/" : parent;
		}

		/**
		 * The workspace-files remote, if this deployment exposes one.
		 *
		 * Two spellings, and the order matters. `ctx.remote.workspaceFiles` is the
		 * natural one, but this plugin declares only `remote.session` in its `inject`,
		 * so cordis's Proxy **throws** on the sibling segment rather than returning
		 * undefined: measured in a real page, that read produced
		 * `cannot get property "remote.workspaceFiles" without inject`. Declaring it
		 * instead would make the whole plugin wait for a remote that a smaller
		 * deployment may not compose — the mentions are not worth that. So the read is
		 * guarded, and `ctx.get(name)` — the same escape hatch that reaches
		 * `chatFileMentions` — is tried second as the service by its own name.
		 *
		 * @param ctx - client root context.
		 * @returns the remote segment, or undefined when it cannot be reached.
		 */
		function workspaceFilesRemote(ctx) {
			try {
				const segment = optionalService(ctx, "remote")?.workspaceFiles;
				if (segment !== undefined && segment !== null && typeof segment.stat === "function") return segment;
			} catch {
				// Not granted on this remote shape; the service lookup below is next.
			}
			try {
				const service = optionalService(ctx, "workspaceFiles");
				if (service !== undefined && service !== null && typeof service.stat === "function") return service;
			} catch (error) {
				reportOnce("workspaceFiles", error);
			}
			return undefined;
		}

		/**
		 * Ask the Host where a path really is, and whether it is still there.
		 *
		 * This is the only authority on both counts. `workspaceFiles.stat` resolves a
		 * relative path against the addressed Session's workspace root — the same
		 * root the Sidebar's own resource loader uses, so it agrees with whatever the
		 * tab is showing — and it explicitly allows absolute paths outside that
		 * workspace, which is where this plugin's artifacts usually live. It answers
		 * `{ absolutePath, version, bytes }` for a file that exists and a
		 * `workspace-file/not-found` result for one that does not.
		 *
		 * Verifying first is not politeness. The native opener cannot report a failure
		 * for a reveal — the Host swallows Explorer's exit code 1, which is what
		 * Explorer returns for a path it cannot select — so "the file is gone" and
		 * "the file opened" reach this plugin as the same success. Measured:
		 * `revealNativePath` on a nonexistent path resolves in 510 ms and shows
		 * nothing at all.
		 *
		 * @param ctx - client root context carrying the session remote.
		 * @param sessionId - Session whose workspace root resolves a relative path.
		 * @param path - the tab's path, absolute or workspace-relative.
		 * @returns `{ found }` plus the Host's absolute path, or undefined when this
		 * deployment cannot answer (an older Host, a remote mid-reconnect) — in which
		 * case the caller proceeds unverified rather than refusing to act.
		 */
		async function locateWorkspaceFile(ctx, sessionId, path) {
			if (typeof sessionId !== "string" || sessionId.length === 0) return undefined;
			try {
				const files = workspaceFilesRemote(ctx);
				if (files === undefined) {
					reportOnce("workspace-files remote", new Error("this deployment exposes no workspaceFiles.stat"));
					return undefined;
				}
				const result = await files.stat(sessionId, path);
				if (result === null || typeof result !== "object") {
					reportOnce("workspaceFiles.stat shape", new Error(`answered ${typeof result}`));
					return undefined;
				}
				if (result.ok !== true) {
					// Only a confirmed absence may stop the action. A directory (this
					// endpoint stats regular files only) and a policy refusal both read
					// as "unreadable", and refusing to open *those* would be worse than
					// the guess this check exists to avoid.
					const code = String(result.error?.code ?? "");
					const message = String(result.error?.message ?? "");
					if (code === "workspace-file/not-found" || /not-found|no entry at/i.test(message)) return { found: false };
					reportOnce(`workspaceFiles.stat(${code === "" ? "no code" : code})`, new Error(message || JSON.stringify(result.error ?? null).slice(0, 200)));
					return undefined;
				}
				const absolutePath = result.value?.absolutePath;
				return {
					found: true,
					absolutePath: typeof absolutePath === "string" && absolutePath.length > 0 ? absolutePath : undefined,
				};
			} catch (error) {
				reportOnce("workspaceFiles.stat", error);
				return undefined;
			}
		}
		 ...
		 * arbitrary tab). It carries **no session**, so it cannot resolve anything —
		 * which is why {@link locateWorkspaceFile} runs first and its `absolutePath`
		 * is what gets opened whenever the Host can supply one.
		 *
		 * ## Why `reveal` opens the folder instead of selecting the file
		 *
		 * The Host's reveal (`explorer.exe /select, <percent-encoded file URL>`) is
		 * unusable in practice, on two measured counts:
		 *
		 *   * **It cannot find non-ASCII names at all.** Explorer does not decode the
		 *     URL the way a browser does, so `暑期科研汇报_v13.pptx` and
		 *     `中文目录\report.pptx` produce no window, while `plain.txt`,
		 *     `with space.txt` (`%20`) and `comma,file.txt` (`%2C`) do open their
		 *     folder. A multi-byte escape is fatal; a single-byte one is not.
		 *   * **Its window does not come to the front.** A reader reported "nothing
		 *     happens", and the same folder was found open six times over: the windows
		 *     were created, behind the browser, one per click. The folder route below
		 *     is the one that reader could actually see.
		 *
		 * So `reveal` asks for the containing folder, which the Host opens with
		 * PowerShell's `Invoke-Item -LiteralPath` — a literal path, so every name
		 * works, and a window that surfaces. The cost is that the file is not
		 * pre-selected; the label promises the folder, and that is what arrives.
		 *
		 * @param ctx - client root context carrying the session remote.
		 ...
		 * @param fallbackAbsolute - the browser's own join, used only when the Host
		 * cannot resolve (`locateWorkspaceFile` answered nothing).
		 * @returns a status message for the user, never a rejection.
		 */
		async function openPathNatively(ctx, sessionId, path, action, fallbackAbsolute) {
			...
			// Verify before acting, because the opener cannot report a reveal's failure.
			// A path that is not there is the one case the plugin can explain exactly,
			// and the explanation is worth more than a success message that is a lie.
			const located = await locateWorkspaceFile(ctx, sessionId, path);
			if (located !== undefined && located.found === false) {
				// Report the path that was actually tried, which is the absolute one the
				// reader can paste into Explorer to see for themselves what moved.
				return `文件不存在：${typeof fallbackAbsolute === "string" && fallbackAbsolute.length > 0 ? fallbackAbsolute : path}`;
			}
			// The Host's own answer wins; the browser's join against the Session root
			// is only the fallback for a deployment that cannot answer at all.
			const target = located?.absolutePath ?? (typeof fallbackAbsolute === "string" && fallbackAbsolute.length > 0 ? fallbackAbsolute : path);
			const folder = action === "reveal" ? parentFolderOf(target) : undefined;
			const request = folder === undefined
				? (action === "reveal" ? { path: target, action } : { path: target })
				: { path: folder };
			try {
				const result = await session.openWorkspacePath(request);
				if (result === null || typeof result !== "object") return "原生打开器没有响应";
				if (result.ok !== true) return `打开失败：${String(result.error?.message ?? "未知原因")}`;
				return NATIVE_ACTION_LABELS[action === "reveal" ? "reveal" : "open"];
			} catch (error) {
		...
		/**
		 * The Session workspace root behind one id, or undefined when it is unknown.
		 *
		 * The menu needs this because a file address may be workspace-relative — the
		 * sidebar's own tree builds some that way, and so does the shipped `present`
		 * vocabulary — while the native opener carries no session and therefore
		 * cannot resolve anything. A relative path handed to it is resolved against
		 * the `dsh web` process's working directory, which is how "put it next to the
		 * launcher" turns into "open the wrong file, or nothing".
		 *
		 * @param ctx - client root context carrying the sessions service.
		 * @param sessionId - the tab's session.
		 * @returns the workspace root, or undefined.
		 */
		function workspaceRootOf(ctx, sessionId) {
			if (typeof sessionId !== "string" || sessionId.length === 0) return undefined;
			try {
				const entry = optionalService(ctx, "sessions")?.list?.getSnapshot?.()?.byId?.[sessionId];
				return entry !== undefined && typeof entry.cwd === "string" && entry.cwd.length > 0 ? entry.cwd : undefined;
			} catch {
				return undefined;
			}
		}

		/** How long a finished action's line stays on screen before the menu closes. */
		const STATUS_LINGER_MS = 1100;
		 ...
		 * actions and calls this registration with the tab and a `dismiss`.
		 *
		 * **Dismissal is deferred, and that is the point.** Both actions are an RPC to
		 * the Host, so their outcome arrives a round trip later — and 0.5.1 dismissed
		 * the menu on click, which unmounted the very line that was supposed to say
		 * what happened. The result was that a refused reveal and a working one looked
		 * identical: nothing. Now the line reports first and the menu closes after
		 * {@link STATUS_LINGER_MS}, which keeps the slot contract (a menu left open
		 * floats over whatever the action replaced) without hiding the answer.
		 *
		...
		function FileTabMenuItems(props) {
			...
			const run = (action) => {
				...
				setBusy(true);
				setStatus("正在打开…");
				const absolute = resolveAgainst(workspaceRootOf(ctx, parts.sessionId), parts.path);
				const finish = (message) => {
					setStatus(message);
					setTimeout(() => owner.dismiss?.(), STATUS_LINGER_MS);
				};
				// The raw tab path goes to the opener, not the browser's join: the Host
				// resolves it against the same Session workspace root the Sidebar used,
				// and only an opener that cannot answer falls back to `absolute`.
				const done = openPathNatively(ctx, parts.sessionId, parts.path, action, absolute);
				// A rejection here is a bug in this plugin, not a Host answer: everything
				// the Host can say comes back as a message. Say so rather than showing a
				// bare label the next reader cannot act on.
				if (typeof done?.then === "function") {
					done.then(finish, (error) => finish(`插件内部错误：${error instanceof Error ? error.message : String(error)}`));
				} else finish("原生打开器没有响应");
			};
		...
		function ensureStyle() {
			...
			style.textContent = [
				 ...
				 * The tab menu's rows.
				 *
				 * The menu renders its own "close" row with the dockkit's CSS-module
				 * class (`_menuItem_*`: `padding:5px 8px`, 13px, `label-primary`,
				 * transparent background, radius 4, `interactive-bg-hover` on hover) and
				 * appends this plugin's items untouched. A bare `<button>` therefore
				 * arrived with the browser's default chrome — grey, inset, centred, its
				 * own font — and read as a foreign block inside the menu. These rules
				 * copy the kit's metrics so all three rows are one list; the selector is
				 * the data attribute this plugin already puts on its own nodes.
				 */
				".dta-menu-action{display:block;box-sizing:border-box;width:100%;margin:0;padding:5px 8px;color:var(--dsw-alias-label-primary);font:inherit;font-size:var(--dsh-content-font-size-secondary,13px);line-height:20px;text-align:left;background:transparent;border:none;border-radius:4px;cursor:pointer}",
				".dta-menu-action:hover{background:var(--dsw-alias-interactive-bg-hover)}",
				".dta-menu-action:focus-visible{outline:none;box-shadow:inset 0 0 0 2px var(--dsw-alias-border-l3)}",
				// The status is a row that is visibly not an option: same metrics, no
				// pointer, dimmed, and able to wrap a full path.
				".dta-menu-status{display:block;padding:5px 8px;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;word-break:break-all;max-width:360px;pointer-events:none}",
			].join("");
		 ...
		 *
		 * `remote.workspaceFiles` is here for one narrow reason, and it is worth the
		 * requirement: it is the only way to ask the Host where a path really is and
		 * whether it still exists. The client context is a Proxy that **throws** on an
		 * undeclared segment, and `ctx.get(name)` does not reach a nested remote
		 * segment either — measured in a real page, both spellings produced
		 * `cannot get property "workspaceFiles" without inject`. The shipped
		 * `dsh-client-ui-sidebar-files` declares exactly this segment plus `remote`,
		 * which is the precedent being followed here.
		 *
		 * The bet is that it exists: every web composition carries the workspace-files
		 * remote (the Sidebar's own file tree is its first consumer), and this plugin
		 * already requires `remote.session` from the same bundle. Verification is what
		 * turns "it says it worked" into "the file is gone, here is the path", and
		 * that is worth requiring.
		 */
		const inject = ["slots", "uiConversation", "remote.session", "remote.workspaceFiles", "sessions"];
		...
		exports.parentFolderOf = parentFolderOf;
