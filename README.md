...
3. **历史窗口自动填满**（顺带，彼此无关）。会话默认只加载最新一页（50 条），长对话在新开页面或 `dsh web` 重启后只看得到最后一屏。本插件会像读者反复点分页那样，把它补到日志开头，上限 60 页。
...
- **历史填充有代价。** 上限 60 页（3000 条消息），每个打开的会话各跑一遍；打开几个长会话就是几百次 RPC 加一次大投影。功能上安全（`loadingOlder` 互斥），但会吃资源。
